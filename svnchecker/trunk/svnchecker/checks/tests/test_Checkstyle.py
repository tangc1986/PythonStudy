...
configString = """
Checkstyle.Java=xxx
Checkstyle.Classpath=xxx
Checkstyle.ConfigFile=%CONFIGFILE%
"""
...
class TestCheckstyle:
    ...
    def setup_class(cls):
        global configString
        if "xxx" in configString:
            py.test.skip("Please configure checkstyle classpath and java path in test_Checkstyle.py.")
        ...
        configString = configString.replace("%CONFIGFILE%", filename)
        cls.configFilename, cls.config = TestConfig().createConfig(configString)

    def testForSuccess(self):
        repository = TestRepository()
        repository.addFile("Test.java", 'public class test { }\n')
        transaction = repository.commit()
        msg, code = Checkstyle.run(transaction, self.config)
        assert code == 0
    ...
    def testForFailure(self):
        ...
        repository.addFile("Test.java", 'public class test { }')
        transaction = repository.commit()
        assert Checkstyle.run(transaction, self.config)[1] == 1
        
    def testForLongJavaFile(self):
        repository = TestRepository()
        repository.addFile("Test.java", longJavaFile)
        transaction = repository.commit()
        msg, code = Checkstyle.run(transaction, self.config)
        assert code == 1
